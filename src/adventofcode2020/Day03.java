...
public class Day03 {
	...
	private static int part1(int right, int down) {

		List<String> bioma = ReadInput.readAsListString("input-day03");

		int treesFound = 0;
		int col = 0;

		for (int i = 0; i < bioma.size(); i += down) {

			char[] line = bioma.get(i).toCharArray();

			if (line[(col % line.length)] == TREE) {
				treesFound++;
			}
		...
		System.out.println("Trees found: " + treesFound);
		return treesFound;
